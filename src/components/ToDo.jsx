...
function Todos() {
    ...
    // State variables for modal handling and editing
    const [modalIsOpen, setModalIsOpen] = useState(false);
    const [currentTodo, setCurrentTodo] = useState(null);
    const [editText, setEditText] = useState('');
    const [editDescription, setEditDescription] = useState('');

    // Function to open modal and populate with current todo's data
    const openModal = (todo) => {
        setCurrentTodo(todo); // Set current todo for editing
        setEditText(todo.title); // Populate title in edit input
        setEditDescription(todo.description); // Populate description in textarea
        setModalIsOpen(true); // Open modal
    };

    // Function to close modal and reset edit state variables
    const closeModal = () => {
    ...
    const handleEditSave = () => {
        ...
        })); // Dispatch update action with edited data
        closeModal(); // Close modal after saving
    };

    // Function to handle deleting a todo with confirmation
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                dispatch(removeTodo(id)); // Dispatch remove action
        ...
        <div className="w-full flex flex-col justify-center items-center">
                    ...
                    >
                        <div className="flex flex-col h-full justify-between">
                            <div className="flex flex-col">
                                {/* Title section with click to toggle status */}
                                <div
                                ...
                                {/* Description and timestamp section */}
                                <div className="text-white mt-2 mb-2 break-all">
                                    {/* Display creation and update timestamps if available */}
                                ...
                                >
                                    ...
                                    {todo.status === 'completed' ? (
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                                            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 0 1-1.043 3.296 3.745 3.745 0 0 1-3.296 1.043A3.745 3.745 0 0 1 12 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 0 1-3.296-1.043A3.746 3.746 0 0 1 3 12c0-1.268.63-2.39 1.593-3.068a3.746 3.746 0 0 1 1.043-3.296 3.746 3.746 0 0 1 3.296-1.043A3.745 3.745 0 0 1 12 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 0 1 3.296 1.043 3.746 3.746 0 0 1 1.043 3.296A3.745 3.745 0 0 1 21 12Z" />
                                        </svg>
                                    ) : (
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
            ...
            >
                <div className="bg-gray-900 p-5 rounded-lg shadow-lg w-full text-white">
                    ...
                    />
                    {/* Button section for cancel and save */}
